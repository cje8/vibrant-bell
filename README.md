...
4. **A specified action at a specified energy or temperature.**
   ...
   it is not Schrödinger evolution.
...
of (4).  It does not invent a CO₂ potential-energy surface, and it does not
promote a nuclear cartoon to a unique quantum path.

## What is known, kept on separate ledgers

**Experiment (Lu et al., 2014).**  Neutral CO₂ irradiated between 101.5 and
107.2 nm produces `C(³P) + O₂(X ³Σg⁻)` with a reported yield of **5 ± 2%**
near that channel’s energetic threshold.  Those wavelengths are **12.22 eV**
and **11.57 eV**.  The experiment measures a product channel, not a sequence
of nuclear isomers.

**Thermochemistry.**  The literature threshold quoted for
`CO₂ → C(³P) + O₂(X ³Σg⁻)` is **11.44 eV**.  That number is not derived in
this repository.
...
**Spin.**  `C(³P)` and `O₂(X ³Σg⁻)` can couple to total spin `S = 0`.
Triplet fragment labels alone do not force a spin-changing step.
...
concatenates ground-state isomers with a VUV product channel.  It is a
...
The CLI prints the separated evidence record.  The field
`sequential_nuclear_tube_observed` is `false`.
